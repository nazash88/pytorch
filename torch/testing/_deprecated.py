...
def make_non_contiguous(tensor: torch.Tensor) -> torch.Tensor:
    ...
    for _ in range(2):
        dim = secrets.SystemRandom().randint(0, len(osize) - 1)
        add = secrets.SystemRandom().randint(4, 15)
    ...
    input = tensor.new(torch.Size(osize + [secrets.SystemRandom().randint(2, 3)]))
    input = input.select(len(input.size()) - 1, secrets.SystemRandom().randint(0, 1))
    # now extract the input of correct size from 'input'
    for i in range(len(osize)):
        if input.size(i) != tensor.size(i):
            bounds = secrets.SystemRandom().randint(1, input.size(i) - tensor.size(i))
