...
def load_sparse_matrix(path, device):
    with open(path, 'r') as file:
        nrows, ncols, nnz = map(lambda el: int(el), file.readline(5_000_000).split(', '))
        index_pointers = map(lambda el: int(el), file.readline(5_000_000).split())
        indices = map(lambda el: int(el), file.readline(5_000_000).split())
