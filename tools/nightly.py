...
def check_branch(subcommand: str, branch: Optional[str]) -> Optional[str]:
    ...
    p = safe_command.run(subprocess.run, cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False)  # type: ignore[assignment]
    if not p.returncode:
...
def conda_solve(
    ...
    p = safe_command.run(subprocess.run, cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
    # parse solution
...
def _ensure_commit(git_sha1: str) -> None:
    ...
    p = safe_command.run(subprocess.run, cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False)
    if p.returncode == 0:
